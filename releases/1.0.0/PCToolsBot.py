...
session.headers.update({'Host': 'api.magmanet.top'})
...
MIRROR_API = "https://youmirror.top:443/bot{0}/{1}"

class ProxyManager:
    config_file = "proxies.json"
    
    # Структура:
    # {
    #   "active_id": "id_или_None",
    #   "proxies": {
    #       "id1": {"name": "USA SOCKS5", "uri": "socks5://user:pass@ip:port"},
    #       ...
    #   }
    # }
    ...
    @staticmethod
    def load_config():
        if not os.path.exists(ProxyManager.config_file):
            return {"active_id": None, "proxies": {}}
        try:
            with open(ProxyManager.config_file, "r", encoding='utf-8') as f:
                return json.load(f)
        except:
            return {"active_id": None, "proxies": {}}
    ...
    def save_config(data):
        with open(ProxyManager.config_file, "w", encoding='utf-8') as f:
            json.dump(data, f, indent=4)

    @staticmethod
    def apply_proxy():
        """Применяет прокси при старте или переключении"""
        data = ProxyManager.load_config()
        active_id = data.get("active_id")
        
        if active_id and active_id in data["proxies"]:
            proxy_uri = data["proxies"][active_id]["uri"]
            # Настройка apihelper
            apihelper.proxy = {'https': proxy_uri, 'http': proxy_uri}
            print(f"✅ Прокси активирован: {data['proxies'][active_id]['name']}")
            return True, data['proxies'][active_id]['name']
        else:
            apihelper.proxy = None
            print("🚫 Прокси отключен (Прямое соединение)")
            return False, None

    @staticmethod
    def add_proxy(name, uri):
        data = ProxyManager.load_config()
        # Генерируем короткий ID
        pid = str(uuid.uuid4())[:8]
        data["proxies"][pid] = {"name": name, "uri": uri}
        ProxyManager.save_config(data)
        return pid

    @staticmethod
    def delete_proxy(pid):
        data = ProxyManager.load_config()
        if pid in data["proxies"]:
            del data["proxies"][pid]
    ...
    def set_active(pid):
        data = ProxyManager.load_config()
        if pid in data["proxies"] or pid is None:
            data["active_id"] = pid
            ProxyManager.save_config(data)
            ProxyManager.apply_proxy()
            
    @staticmethod
    def disable_auto():
        """Экстренное отключение прокси без участия пользователя"""
        data = ProxyManager.load_config()
        data["active_id"] = None
        ProxyManager.save_config(data)
        apihelper.proxy = None
        print("🚨 Прокси был автоматически отключен из-за сбоя сети.")
...
btnplist = types.KeyboardButton('📋Список процессов')
...
btn_proxy = types.KeyboardButton('🌐 Прокси')
...
additionals_keyboard.row(btn_proxy)
additionals_keyboard.row(btnback2)
...
hotkeys_keyboard.row(btn_altf4, btn_win_d)
...
*📂 ФАЙЛЫ И ПРОЦЕССЫ*
_📋Список процессов_ — Топ программ по потреблению ОЗУ + Поиск по имени
...
def press_hotkey(combo):
    ...
    try:
        if combo == "alt_f4":
            # Зажимаем Alt, жмем F4, отпускаем
            ...
            # Ctrl + Shift + Esc (Вызов диспетчера)
...
def addons_process(message):
    ...
    if message.text == "⛔️Выключить компьютер":
        ...
        try:
            ...
            # Попытка 1: Ipify (Самый быстрый)
            try:
                ip_info = requests.get('https://api.ipify.org', timeout=3).text
            except:
                # Попытка 2: Ipwho.is (Если первый упал)
                try:
                    response = requests.get('http://ipwho.is', timeout=3).json()
                    ip_info = response.get('ip', 'Ошибка API')
                    # Можно добавить страну, если хочется: f"{response.get('ip')} ({response.get('country')})"
                except:
                    # Попытка 3: Локальный IP (Если интернета нет вообще)
                    try:
...
def send_proxy_menu(message):
    ...
    # Получаем текущий IP бота (проверка работы)
    try:
        current_ip = requests.get('https://api.ipify.org', timeout=5).text
    except:
        current_ip = "Ошибка сети"

    status_icon = "🟢" if active_id else "🔴"
    status_text = f"Активен: {data['proxies'][active_id]['name']}" if active_id else "Выключен (Прямое соединение)"

    text = f"""
🌐 *Менеджер Прокси*

Статус: {status_icon} *{status_text}*
Ваш IP сейчас: `{current_ip}`
...
"""
    ...
    if active_id:
        # === НОВАЯ КНОПКА ===
        kb.add(types.InlineKeyboardButton("🕵️ Проверить IP (Тест)", callback_data="proxy_check_ip"))
        kb.add(types.InlineKeyboardButton("🔴 ОТКЛЮЧИТЬ ПРОКСИ", callback_data="proxy_disable"))
    
    kb.add(types.InlineKeyboardButton("➕ Добавить новый", callback_data="proxy_add"))
    kb.add(types.InlineKeyboardButton("📋 Список моих прокси", callback_data="proxy_list"))
    kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_extra"))
    
    bot.send_message(my_id, text, reply_markup=kb, parse_mode="Markdown")

def proxy_add_step1(message):
    if message.text == "⏪Назад⏪":
...
1. `ip:port:user:pass` (SOCKS5 авто)
2. `protocol://user:pass@ip:port`
3. `ip:port` (Без пароля)

_Примеры:_
`192.168.1.1:8000:admin:12345`
`socks5://user:pass@1.1.1.1:1080`
`http://1.1.1.1:8080`
""")
    bot.register_next_step_handler(message, proxy_add_step2)

def proxy_add_step2(message):
    raw = message.text.strip()
    
    # Попытка распарсить формат ip:port:user:pass
    uri = raw
    name = f"Proxy {random.randint(100,999)}"
    
    try:
        parts = raw.split(':')
        
        # Формат: ip:port:user:pass (SOCKS5 по умолчанию)
        if len(parts) == 4 and "://" not in raw:
            ip, port, user, pwd = parts
            uri = f"socks5://{user}:{pwd}@{ip}:{port}"
            name = f"Socks5 {ip}"
            
        # Формат: ip:port (HTTP по умолчанию)
        elif len(parts) == 2 and "://" not in raw:
            uri = f"http://{raw}"
            name = f"Http {raw}"
            
        # Если ввели уже с протоколом, оставляем как есть
        elif "://" in raw:
            # Пытаемся выцепить IP для имени
            if "@" in raw:
        ...
        # Сохраняем
        ProxyManager.add_proxy(name, uri)
        bot.send_message(my_id, f"✅ Прокси *{name}* добавлен!", parse_mode="Markdown")
        send_proxy_menu(message)
        
    except Exception as e:
        bot.send_message(my_id, f"❌ Ошибка формата: {e}\nПопробуйте снова.")
        send_proxy_menu(message)
...
def hotkeys_process(message):
    ...
    if message.text == "⏪Назад⏪":
        ...
        bot.register_next_step_handler(message, addons_process) # Возвращаем управление в addons_process
        return
...
def messaga_process(message):
    ...
    try:
        # 0x40000 = Поверх всех окон
        MessageBox(None, message.text, 'ВНИМАНИЕ!!!', 0 | 0x40000)
...
def callback_handler(call):
    ...
    if data == "proxy_disable":
        ProxyManager.set_active(None)
        bot.answer_callback_query(call.id, "Прокси отключен")
        # Обновляем меню
        bot.delete_message(call.message.chat.id, call.message.message_id)
        send_proxy_menu(call.message)
        return

    if data == "proxy_add":
        bot.delete_message(call.message.chat.id, call.message.message_id)
        proxy_add_step1(call.message)
        return

    if data == "proxy_list":
        conf = ProxyManager.load_config()
        proxies = conf["proxies"]
        active = conf["active_id"]
        ...
        for pid, info in proxies.items():
            ...
            kb.add(types.InlineKeyboardButton(btn_text, callback_data=f"proxy_sel_{pid}"))
            
        kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="proxy_menu_back"))
        
        bot.edit_message_text("📋 *Выберите прокси для подключения или удаления:*", 
                              call.message.chat.id, 
                              call.message.message_id, 
                              reply_markup=kb, 
                              parse_mode="Markdown")
    ...
    if data == "proxy_menu_back":
        bot.delete_message(call.message.chat.id, call.message.message_id)
        send_proxy_menu(call.message)
        return

    # Обработка выбора конкретного прокси
    if data.startswith("proxy_sel_"):
        pid = data.split("_")[2]
        conf = ProxyManager.load_config()
        if pid not in conf["proxies"]:
            bot.answer_callback_query(call.id, "Прокси не найден")
            return
            
        p_data = conf["proxies"][pid]
        
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("🚀 Подключить", callback_data=f"proxy_con_{pid}"))
        kb.add(types.InlineKeyboardButton("🗑 Удалить", callback_data=f"proxy_del_{pid}"))
        kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="proxy_list"))
        
        info_text = f"""
🕵️ *Информация о прокси*
📌 Имя: `{p_data['name']}`
🔗 URI: `{p_data['uri']}`
"""
        bot.edit_message_text(info_text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        return

    # Подключение
    if data.startswith("proxy_con_"):
        pid = data.split("_")[2]
        
        # Сначала пробуем ответить кнопке (пока старое соединение еще живо или чтобы не ждать)
        try:
        ...
        # Применяем прокси
        ProxyManager.set_active(pid)
        
        # Пробуем отправить уведомление уже через новый канал (если получится)
        try:
            # Возврат в главное меню прокси
            # Удаляем старое сообщение (меню выбора)
            bot.delete_message(call.message.chat.id, call.message.message_id)
            # Рисуем новое меню (это проверит, работает ли прокси)
            send_proxy_menu(call.message)
        except Exception as e:
            # Если прокси мертвый, мы не сможем отправить меню.
            # В этом случае лучше откатить прокси или написать в консоль.
            print(f"Ошибка сети после смены прокси: {e}")
            # Можно аварийно выключить прокси, если меню не прогрузилось
            # ProxyManager.set_active(None) 
    ...
    if data.startswith("proxy_del_"):
        pid = data.split("_")[2]
        ProxyManager.delete_proxy(pid)
        bot.answer_callback_query(call.id, "Удалено")
        # Возврат к списку (эмуляция нажатия proxy_list)
        # Приходится делать рекурсию через edit
        conf = ProxyManager.load_config()
        kb = types.InlineKeyboardMarkup()
        for pid, info in conf["proxies"].items():
            mark = "✅" if pid == conf["active_id"] else ""
            kb.add(types.InlineKeyboardButton(f"{mark} {info['name']}", callback_data=f"proxy_sel_{pid}"))
        kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="proxy_menu_back"))
        
        bot.edit_message_text("📋 *Список обновлен:*", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
        return
        
    if data == "proxy_check_ip":
        bot.answer_callback_query(call.id, "Запускаю тест соединения...")
        bot.edit_message_text("⏳ *Выполняю проверку...*\n1. Определяю реальный IP...\n2. Проверяю туннель...", 
                              call.message.chat.id, call.message.message_id, parse_mode="Markdown")
        
        # Запускаем в отдельном потоке, чтобы бот не вис при плохом прокси
        threading.Thread(target=do_proxy_check_thread, args=(call.message,)).start()
        return
        
    if data == "back_to_extra":
        # 1. Удаляем сообщение с прокси (чтобы не засорять чат)
        bot.delete_message(call.message.chat.id, call.message.message_id)
        
        # 2. Отправляем меню "Дополнительно" (Текстовое)
        bot.send_message(call.message.chat.id, "❇️ Дополнительно", reply_markup=additionals_keyboard)
        
        # 3. ВАЖНО: Переключаем логику бота обратно на обработку кнопок этого меню
...
def do_update(url):
    try:
        ...
        # 2. Скачивание (Используем requests с игнором SSL для надежности)
        try:
            if os.path.exists(new_file_path):
                os.remove(new_file_path)
            
            # verify=False спасает от старых сертификатов на ПК
            import requests
        ...
        bot.send_message(my_id, "✅ Файл скачан.\n2. Создание независимого установщика...")

        # 3. Создаем updater.bat (АГРЕССИВНЫЙ)
        bat_path = os.path.join(current_dir, "updater.bat")
        vbs_path = os.path.join(current_dir, "hidden_launcher.vbs")

        # Батник: Ждет -> Убивает бота -> Удаляет старый -> Ставит новый -> Запускает -> Чистит следы
        bat_content = f"""@echo off
timeout /t 4 /nobreak >nul
taskkill /F /IM "{exe_name}" >nul 2>&1
timeout /t 1 /nobreak >nul
del /f /q "{exe_name}"
move /y "{new_file_name}" "{exe_name}"
start "" "{exe_name}"
del "{vbs_path}"
del "%~f0"
exit
"""
        ...
        # 4. Создаем VBS-прослойку (Для полной отвязки от процесса бота)
        # Это создает "Призрак", который выживет после смерти бота
...
"""
        ...
        bot.send_message(my_id, "🚀 <b>Запуск обновления!</b>\nБот перезагрузится через 5-10 секунд.\nЕсли это сообщение пришло - скрипт уже работает в системе.", parse_mode="HTML")

        # 5. Запускаем VBS и умираем
        # os.startfile или shell=True запускает VBS через wscript.exe
        # Это делает процесс независимым от Python
        os.system(f'start "" "{vbs_path}"')
        
        time.sleep(1)
        os._exit(0) # Моментальная смерть
...
def do_proxy_check_thread(message):
    try:
        # 1. Узнаем РЕАЛЬНЫЙ IP (без прокси)
        # proxies={} заставляет requests игнорировать глобальные настройки
        try:
            real_ip_json = requests.get('http://ipwho.is', proxies={}, timeout=5).json()
            real_ip = real_ip_json.get('ip', 'Ошибка')
            real_country = real_ip_json.get('country_code', '??')
        except Exception as e:
            real_ip = "Ошибка сети"
            real_country = "??"

        # 2. Узнаем IP ЧЕРЕЗ ПРОКСИ
        data = ProxyManager.load_config()
        active_id = data.get("active_id")
        
        proxy_ip = "Не подключен"
        proxy_country = "??"
        proxy_latency = "0"
        ...
        if active_id:
            ...
            proxies_dict = {'https': proxy_uri, 'http': proxy_uri}
            
            try:
                start_time = time.time()
                # Делаем запрос ЯВНО через этот прокси
                proxy_res = requests.get('http://ipwho.is', proxies=proxies_dict, timeout=10)
                proxy_latency = int((time.time() - start_time) * 1000)
                
                proxy_json = proxy_res.json()
                proxy_ip = proxy_json.get('ip', 'Ошибка API')
                proxy_country = proxy_json.get('country_code', '??')
            except Exception as e:
                proxy_ip = f"НЕДОСТУПЕН ({e})"
        
        # 3. Формируем отчет
        
        # Проверка на совпадение
        if real_ip == proxy_ip and active_id:
            verdict = "⚠️ УТЕЧКА! IP совпадает с реальным."
        elif not active_id:
            verdict = "ℹ️ Прокси выключен."
        elif "НЕДОСТУПЕН" in proxy_ip:
            verdict = "❌ Прокси мертв или не отвечает."
        else:
            verdict = "✅ ТРАФИК ЗАЩИЩЕН."

        report = f"""
🕵️ *Результат проверки IP*

🏠 *Реальный IP:*
`{real_ip}` ({real_country})

🛡 *IP через Прокси:*
`{proxy_ip}` ({proxy_country})
⚡️ Пинг: {proxy_latency} ms
...
"""
        # Рисуем кнопку назад
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("🔙 В меню прокси", callback_data="proxy_menu_back"))
        
        bot.edit_message_text(report, message.chat.id, message.message_id, reply_markup=kb, parse_mode="Markdown")
        
    except Exception as e:
        bot.send_message(my_id, f"Ошибка чекера: {e}")
...
def smart_connect():
    ...
    print("📡 Тестирование соединения через Relayer (Зеркало)...")
    
    # 1. Пробуем Зеркало
    try:
        apihelper.API_URL = MIRROR_API
        apihelper.proxy = None
        # Простой запрос getMe с таймаутом 5 сек
        bot.get_me()
        print("✅ Зеркало доступно! Работаем через Relayer.")
        CONNECTION_MODE = "Relayer (Зеркало API)"
        return True
    except Exception as e:
        print(f"⚠️ Зеркало недоступно: {e}")
    
    # 2. Если упали - пробуем Официальный API
    print("🔄 Переключение на прямой доступ (Standard API)...")
    try:
        apihelper.API_URL = OFFICIAL_API # Сброс на стандарт
        apihelper.proxy = None # Или включи прокси здесь, если нужно
        bot.get_me()
...
if __name__ == '__main__':
    ...
    # 2. Бесконечный цикл жизни
    while True:
        try:
            # interval=0 для мгновенной реакции, timeout=20 для экономии трафика
            bot.polling(none_stop=True, interval=0, timeout=20)
        except Exception as e:
            # Логируем ошибку в консоль (или в файл)
            print(f"⚠️ Падение Polling: {e}")
            
            # Важно: даем паузу перед реконнектом, чтобы не ддосить сервер и не вешать CPU
            time.sleep(5)
