...
MIRROR_API = "https://api.youmirror.top:443/bot{0}/{1}"
...
class WGManager:
        ...
        if os.path.exists(cls.storage_file):
            try:
                with open(cls.storage_file, "r", encoding="utf-8") as f:
                    return json.load(f).get("wg_conf")
            except:
                pass
        return None
    ...
    def apply_config(cls, conf_text, save=True):
        ...
        clean_conf = "\n".join([line for line in conf_text.split('\n') if "[Socks5]" not in line and "BindAddress" not in line])
        modified_conf = f"{clean_conf}\n\n[Socks5]\nBindAddress = 127.0.0.1:10808\n"
        
        # Сохраняем во временную папку (%TEMP%)
        ...
        try:
            ...
            res = clean_session.get(test_url, timeout=10)
            ...
            if res.status_code == 200:
                ...
                return True, "✅ Трафик переведен в WireGuard. Следы скрыты."
            else:
                cls.stop()
                return False, f"❌ API вернул код: {res.status_code}"
                
        except Exception as e:
            cls.stop()
            return False, f"⚠️ WG-туннель мертв (Таймаут).\nОшибка: {e}"
...
def smart_connect():
    ...
    # ==========================================
    # ШАГ 1: Тест Зеркала (Relayer API)
    # ==========================================
    print("📡 [1/3] Тестирование соединения через Relayer (Зеркало)...")
    try:
        WGManager.stop() # На всякий случай гасим туннели
        
        mirror_session = requests.Session()
        mirror_session.verify = False
        # Здесь впишите ДОМЕН ВАШЕГО ЗЕРКАЛА
        mirror_session.headers.update({'Host': 'api.magmanet.top'}) 
        
        apihelper.SESSION = mirror_session
    ...
    # ==========================================
    # ШАГ 2: Тест WireGuard (Сохраненный конфиг)
    # ==========================================
    ...
    try:
        if WGManager.load_config():
            ...
            if success:
                print("✅ WireGuard успешно поднялся в фоне!")
                CONNECTION_MODE = "WireGuard Tunnel"
                return True
            else:
    ...
    # ==========================================
    # ШАГ 3: Прямой доступ (Standard API)
    # ==========================================
